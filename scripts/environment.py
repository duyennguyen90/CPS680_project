...
class Turn(object):
    ...
    def action(self, player, opponent):
        ...
        if player_act.card_play.value == "PL4":
            self.action_plus(player   = player_act, 
                             opponent = player_pas, 
                             penalty  = 4,
                             color = player_act.card_play.color)
    ...
    def action_plus(self, player, opponent, penalty,color):
        """
        ...
        while hit == True:
            ...
            for card in player_pas.hand:
                if card.value == "PL"+str(penalty) and card.color == color:
